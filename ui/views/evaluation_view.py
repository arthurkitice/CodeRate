...
class EvaluationView(DashboardFormView):
    ...
    def edit_criteria(self, criteria_id): #ANALISAR NECESSIDADE DE EDITAR AVALIAÇÃO AQUI
        self.on_criteria_edit(criteria_id=criteria_id, user_id=self.user_id)

    def delete_criteria(self, criteria_id): #DELETAR AVALIAÇÃO AQUI
        try:
            with get_db() as db:
    ...
    def build_criteria_buttons(self, row=0, column=0): #IMPLEMENTAR BOTÕES DE AVALIAÇÕES PASSADAS AQUI
    ...
    def build_criteria_info(self, criteria, row=2, column=1): #CRIAR FRAME DAS AVALIAÇÕES PASSADAS AQUI
